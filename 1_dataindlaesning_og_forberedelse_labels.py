...
labels_mapping = {
    ...
    'e.coli': 6,
    'k._pneumonia': 7,
    'Morganella_Morganii': 8,
    'P._aerigimosa': 9,
    'S._aureus': 10,
    's._saprofyticus': 11,
    'stapholococcus_coagulans': 12
}
...
def process_spectral_data(json_filepath, mrspectra_filepath):
    try:
        ...
        if spectra_data is not None:
            ...
            # Bestem label baseret på filnavnet
            label = -1
            for name in labels_mapping.keys():
                if name in json_filepath:
                    label = labels_mapping[name]
                    break

            output_filename = f"output_spectra_{os.path.basename(json_filepath).replace('.json', '')}_label_{label}.csv"
            with open(output_filename, 'w', newline='') as csvfile:
                csvwriter = csv.writer(csvfile)
                csvwriter.writerow(['Wavenumber (cm-1)', 'Cleaned Intensity', 'Label'])
                for i in range(len(wavenumbers[valid_range])):
                    csvwriter.writerow([wavenumbers[valid_range][i], spectrum_cleaned[i], label])
