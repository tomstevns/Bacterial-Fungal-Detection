...
# Funktion til at omdøbe jpg-filer
    ...
    for root, dirs, files in os.walk(directory):
        for filename in files:
            if filename.endswith('.jpg'):
                ...
                if old_filepath != new_filepath:
                    print(f"Omdøber JPG-fil {old_filepath} til {new_filepath}")
                    os.rename(old_filepath, new_filepath)
...
# Funktion til at behandle og gemme spektraldata
def process_spectral_data(json_filepath, mrspectra_filepath, expected_length=2101):
    ...
    if spectra_data is not None:
        ...
        # Interpolér til forventet længde
        ...
        with open(output_filename, 'w', newline='') as csvfile:
            ...
            for i in range(len(wavenumbers[valid_range])):
                csvwriter.writerow([wavenumbers[valid_range][i], spectrum_interpolated[i]])
...
for base_dir in base_dirs:
    ...
    rename_jpg_files_in_directory(base_dir)  # Omdøb JPG-filer
...
for base_dir in base_dirs:
    ...
    for root, dirs, files in os.walk(base_dir):
        ...
        for json_file in json_files:
            ...
            # Find den tilsvarende mrspectra fil
