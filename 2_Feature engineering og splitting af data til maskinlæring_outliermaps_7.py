...
# Definer base directories og labels mapping
...
# Funktion til at fjerne baggrundsstøj og isolere bakteriekolonierne
def remove_background_and_isolate_colonies(intensity_values):
    q1, q3 = np.percentile(intensity_values, [25, 75])
    iqr = q3 - q1
    lower_bound = q1 - 1.5 * iqr
    upper_bound = q3 + 1.5 * iqr
    # Fjern data uden for dette interval som outliers
    filtered_values = np.clip(intensity_values, lower_bound, upper_bound)
    return filtered_values

# Funktion til at lave data augmentation ved at tilføje støj
...
def process_directory(base_dir, set_name):
    ...
    for filename in os.listdir(base_dir):
        ...
        if filename.endswith(".csv"):
            ...
            if 'Cleaned Intensity' in df.columns:
                ...
                # Baggrundsudrensning og isolering af bakteriekolonier
                filtered_intensity = remove_background_and_isolate_colonies(cleaned_intensity)

                if len(filtered_intensity) < expected_length:
                    x_old = np.linspace(0, len(filtered_intensity) - 1, len(filtered_intensity))
                    f = interp1d(x_old, filtered_intensity, kind='linear', fill_value='extrapolate')
                    x_new = np.linspace(0, len(filtered_intensity) - 1, expected_length)
                    normalized_smoothed = f(x_new)
                else:
                    normalized_smoothed = filtered_intensity[:expected_length]
    ...
    for key, count in samples_count.items():
        if count < min_samples_per_class:
            ...
            print(f"Klasse '{key}' i {set_name} har kun {count} samples, udfører data augmentering...")
