...
param_grid = {
    'learning_rate': [0.0005, 0.0001],  # Experimenting with smaller learning rates
    'dropout_rate': [0.4, 0.5],  # Regularization with dropout
    'filters': [32, 64],  # Increasing model complexity
    'rotation_range': [40, 50],  # Aggressive data augmentation
    'zoom_range': [0.2, 0.3],  # Aggressive data augmentation
    'epochs': [10, 15]  # Adjusted based on overfitting patterns
}
...
def create_model(learning_rate, dropout_rate, filters, num_classes):
    model = tf.keras.models.Sequential([
        ...
        tf.keras.layers.Dense(256, activation='relu',
                              kernel_regularizer=tf.keras.regularizers.l2(0.001)),  # Increased dense units
...
for params in param_combinations:
    ...
    callbacks = [
        ...
        EarlyStopping(monitor='val_loss', patience=2, restore_best_weights=True)  # Early stopping for overfitting prevention
    ]

    # Train the model
    history = model.fit(
        train_generator,
        epochs=epochs,  # Adjusted to 10 or 15 depending on the grid search
