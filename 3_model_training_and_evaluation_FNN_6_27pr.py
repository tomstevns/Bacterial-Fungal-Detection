...
param_grid = {
    'learning_rate': [0.0001, 0.00001],  # Starting with smaller learning rates for smoother convergence
    'dropout_rate': [0.7, 0.9],  # Higher dropout rates to prevent overfitting
    'filters': [8, 16],  # Reducing model complexity by reducing filters
    'rotation_range': [70],  # More aggressive data augmentation
    'zoom_range': [0.4],  # Continued aggressive data augmentation
    'epochs': [40, 50]  # More epochs to allow better learning and fine-tuning
}
...
def create_model(learning_rate, dropout_rate, filters, num_classes):
    ...
    model = tf.keras.models.Sequential([
        # Higher L2 regularization to counteract overfitting
        tf.keras.layers.Conv2D(filters, (3, 3), activation='relu', input_shape=(150, 150, 3),
                               kernel_regularizer=tf.keras.regularizers.l2(0.015)),  # Further increased L2 regularization
        ...
        tf.keras.layers.Dropout(dropout_rate),  # Increased dropout for regularization
        tf.keras.layers.Flatten(),
        tf.keras.layers.Dense(256, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.015)),
...
for params in param_combinations:
    ...
    train_datagen = ImageDataGenerator(
        ...
        width_shift_range=0.3,  # More aggressive shifts to prevent overfitting
