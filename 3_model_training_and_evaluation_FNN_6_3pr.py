...
param_grid = {
    'learning_rate': [0.001, 0.0005],  # Experimenting with different learning rates
    'dropout_rate': [0.3, 0.5],  # Regularization with dropout
    'filters': [32, 64],  # Different number of filters in convolution layers
    'rotation_range': [20, 40],  # Data augmentation parameters
    'zoom_range': [0.2, 0.3],  # Data augmentation parameters
    'epochs': [10, 30]  # Reducing epochs based on overfitting
}
...
def create_model(learning_rate, dropout_rate, filters):
    model = tf.keras.models.Sequential([
        tf.keras.layers.Conv2D(filters, (3, 3), activation='relu', input_shape=(150, 150, 3), kernel_regularizer=tf.keras.regularizers.l2(0.001)),
        tf.keras.layers.MaxPooling2D(2, 2),
        tf.keras.layers.Conv2D(filters * 2, (3, 3), activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.001)),
        tf.keras.layers.MaxPooling2D(2, 2),
        tf.keras.layers.Conv2D(filters * 4, (3, 3), activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.001)),
        tf.keras.layers.MaxPooling2D(2, 2),
...
for params in param_combinations:
    ...
    history = model.fit(
        train_generator,
        epochs=epochs,  # Adjusted to 10 or 30 depending on the grid search
