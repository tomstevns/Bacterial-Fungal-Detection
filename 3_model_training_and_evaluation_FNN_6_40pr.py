...
param_grid = {
    ...
    'dropout_rate': [0.5, 0.6],  # Slightly increased dropout rate to prevent overfitting
    'filters': [32, 64],
    'rotation_range': [40, 50],
    'zoom_range': [0.2, 0.3],
    'epochs': [25, 30]  # Increased epoch range for more training with early stopping
}
...
def create_model(learning_rate, dropout_rate, filters, num_classes):
    model = tf.keras.models.Sequential([
        ...
        tf.keras.layers.Dense(256, activation='relu',
                              kernel_regularizer=tf.keras.regularizers.l2(0.02)),  # Dense units with L2
...
for params in param_combinations:
    ...
    # Callbacks
    callbacks = [
        LearningRateScheduler(scheduler, verbose=1),  # Dynamically adjust learning rate
