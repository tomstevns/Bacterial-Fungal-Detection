...
param_grid = {
    ...
    'dropout_rate': [0.5, 0.6],  # Increased dropout rate to prevent overfitting
    'filters': [32, 64],
    'rotation_range': [30, 40],  # Reduced rotation range for controlled augmentation
    'zoom_range': [0.1, 0.2],  # Reduced zoom range
    'epochs': [15, 20]  # Slightly increased epoch range with early stopping
}
...
for params in param_combinations:
    ...
    train_generator = train_datagen.flow_from_directory(
        train_data_dir, target_size=(150, 150), batch_size=16, class_mode='categorical'  # Reduced batch size to 16
    )
    val_generator = val_datagen.flow_from_directory(
        val_data_dir, target_size=(150, 150), batch_size=16, class_mode='categorical'  # Reduced batch size to 16
    )
    ...
    # Callbacks
    callbacks = [
        ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=6, min_lr=0.00001),  # Patience increased to 6
        EarlyStopping(monitor='val_loss', patience=6, restore_best_weights=True),  # Patience increased to 6
