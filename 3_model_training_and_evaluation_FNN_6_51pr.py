...
param_grid = {
    'learning_rate': [0.00005, 0.00001],  # Reduced learning rates for finer gradient steps
    'dropout_rate': [0.6, 0.7],  # Increased dropout rate to prevent overfitting
    'filters': [32, 64],  # Experimenting with a range of filters
    'rotation_range': [20, 30],  # More aggressive augmentations
    'zoom_range': [0.1, 0.2],  # Slightly increased zoom range
    'epochs': [25, 30]  # Increased epoch range
}
...
# Function to create the CNN model for classification (output layer must match the number of classes)
def create_feature_extractor(learning_rate, dropout_rate, filters, num_classes):
    ...
    # Add the fully connected layer for classification (num_classes should match the number of classes)
...
for params in param_combinations:
    ...
    model = create_feature_extractor(learning_rate, dropout_rate, filters, num_classes)

    # Callbacks
    callbacks = [
        ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=6, min_lr=1e-7),  # Adjust patience and min_lr
