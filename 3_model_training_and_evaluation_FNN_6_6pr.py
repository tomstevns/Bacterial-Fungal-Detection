...
param_grid = {
    'learning_rate': [0.001, 0.0005, 0.0001],  # Experimenting with smaller learning rates
    'dropout_rate': [0.3, 0.5],  # Regularization with dropout
    'filters': [16, 32],  # Reducing model complexity to avoid overfitting
    'rotation_range': [40, 60],  # More aggressive data augmentation
    'zoom_range': [0.3, 0.4],  # More aggressive data augmentation
    'epochs': [10, 20]  # Reducing epochs further based on overfitting patterns
}
...
# Check data integrity function (unchanged)
...
def create_model(learning_rate, dropout_rate, filters):
    model = tf.keras.models.Sequential([
        tf.keras.layers.Conv2D(filters, (3, 3), activation='relu', input_shape=(150, 150, 3), kernel_regularizer=tf.keras.regularizers.l2(0.01)),  # Increased L2 regularization
        tf.keras.layers.MaxPooling2D(2, 2),
        tf.keras.layers.Conv2D(filters * 2, (3, 3), activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.01)),
        tf.keras.layers.MaxPooling2D(2, 2),
        tf.keras.layers.Dropout(dropout_rate),  # Dropout to prevent overfitting
        tf.keras.layers.Flatten(),
        tf.keras.layers.Dense(256, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.01)),  # Reduced dense units
...
for params in param_combinations:
    ...
    model = create_model(learning_rate, dropout_rate, filters)

    # Callbacks
    callbacks = [
        ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=3, min_lr=0.00001),  # Lower learning rate on plateau
        EarlyStopping(monitor='val_loss', patience=1, restore_best_weights=True)  # Early stopping for overfitting prevention
    ]

    # Train the model
    history = model.fit(
        train_generator,
        epochs=epochs,  # Adjusted to 10 or 20 depending on the grid search
    ...
    # Log the results
    log_results(dict(learning_rate=learning_rate, dropout_rate=dropout_rate, filters=filters, rotation_range=rotation_range, zoom_range=zoom_range, epochs=epochs), test_acc, val_loss)
