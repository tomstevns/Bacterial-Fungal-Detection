...
import joblib  # For saving and loading GridSearchCV state
...
checkpoint_file = 'gridsearch_checkpoint.pkl'  # File to store intermediate results

# Define expected classes and minimum samples
expected_classes = ['Aero', 'Bacillus', 'Bor', 'E', 'Morganella', 'stapholococcus']
min_samples = 2

# Function to save GridSearchCV results
def save_checkpoint(grid_search, filename=checkpoint_file):
    joblib.dump(grid_search, filename)
    print(f"Saved checkpoint to {filename}")

# Function to load GridSearchCV results
...
grid_search = load_checkpoint()
if grid_search is None:
    print("No existing checkpoint found, starting a new GridSearchCV.")
...
try:
    ...
    save_checkpoint(grid_search)  # Save when interrupted
