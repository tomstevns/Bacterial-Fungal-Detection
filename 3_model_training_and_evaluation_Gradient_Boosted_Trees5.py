...
if best_gbt is None:
    ...
    param_grid = {
        ...
        'learning_rate': [0.01, 0.05],  # Narrow down based on initial results
        'subsample': [0.6, 0.7, 0.8],  # Focus on smaller subsample values
        'min_samples_split': [2, 5, 10],  # Experiment with larger values to avoid overfitting
        'min_samples_leaf': [1, 2, 3]  # Larger values for more generalization
    }
    ...
    # Create a GridSearchCV object with higher verbosity and 5-fold cross-validation
    grid_search = GridSearchCV(estimator=gbt, param_grid=param_grid, cv=5, n_jobs=-1, verbose=3)

    # Fit the model, with intermediate checkpoints
    try:
        ...
        for i in range(1, 6):
            fold_results = results[results['split{}_test_score'.format(i - 1)].notnull()]
            fold_results.to_csv(f'grid_search_fit_results_fold_{i}.csv', index=False)
            print(f"Results for fold {i} saved to 'grid_search_fit_results_fold_{i}.csv'")

        # Save the consolidated results for all folds
        results.to_csv('grid_search_fit_results_all_folds.csv', index=False)
        print("All fit results saved to 'grid_search_fit_results_all_folds.csv'")
