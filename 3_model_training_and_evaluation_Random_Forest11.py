...
def save_model(model, filename=model_checkpoint_file):
    if isinstance(model, (RandomForestClassifier, GradientBoostingClassifier, VotingClassifier)):
...
def load_model(filename=model_checkpoint_file):
    if os.path.exists(filename):
        ...
        if isinstance(loaded_model, (RandomForestClassifier, GradientBoostingClassifier, VotingClassifier)):
            return loaded_model
        else:
...
# Apply aggressive SMOTE to balance classes
print("Applying aggressive SMOTE to balance classes...")
smote = SMOTE(random_state=42, sampling_strategy='auto')  # Automatically balance all classes
...
if best_model is None:
    # Define a parameter grid for RandomForest and GradientBoosting hyperparameter tuning
    rf_param_grid = {
    ...
    # Create RandomForestClassifier and GradientBoostingClassifier
    rf = RandomForestClassifier(random_state=42)
    gbt = GradientBoostingClassifier(random_state=42)

    # Use 3-fold cross-validation for both models
    rf_grid_search = GridSearchCV(estimator=rf, param_grid=rf_param_grid, cv=3, n_jobs=-1, verbose=3)
    gbt_grid_search = GridSearchCV(estimator=gbt, param_grid=gbt_param_grid, cv=3, n_jobs=-1, verbose=3)
    ...
    try:
        ...
        gbt_grid_search.fit(X_train_reduced, y_train_encoded)

        # Combine models into VotingClassifier
        best_rf = rf_grid_search.best_estimator_
        best_gbt = gbt_grid_search.best_estimator_

        # Create an ensemble using VotingClassifier
        best_model = VotingClassifier(estimators=[('rf', best_rf), ('gbt', best_gbt)], voting='soft')
