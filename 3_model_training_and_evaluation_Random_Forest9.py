...
# Aggressive augmentation for Bacillus, Bor, and E
def augment_image(img, img_size, is_bacillus=False, is_bor=False, is_e=False, is_staph=False):
    # Apply random augmentations
    img_rotated = rotate(img, angle=random.uniform(-30, 30), mode='wrap')  # Rotate image
    img_brightened = exposure.adjust_gamma(img_rotated, gamma=random.uniform(0.5, 2.0))  # Adjust brightness
    if is_bacillus or is_bor or is_e or is_staph:  # Apply more augmentation for Bacillus, Bor, E, and Stapholococcus
        img_brightened = resize(img_brightened,
                                (img_brightened.shape[0] + 10, img_brightened.shape[1] + 10))  # Zoom effect
    img_resized = resize(img_brightened, img_size)  # Resize back to the desired size
    return img_resized


# Function to load images and labels with class-specific aggressive augmentation
    ...
    for cls in expected_classes:
        ...
        for img_name in os.listdir(class_dir):
            ...
            try:
                ...
                if augment:
                    is_bacillus = cls == 'Bacillus'
                    is_bor = cls == 'Bor'
                    is_e = cls == 'E'
                    is_staph = cls == 'stapholococcus'
                    img_resized = augment_image(img_resized, img_size, is_bacillus=is_bacillus, is_bor=is_bor,
                                                is_e=is_e, is_staph=is_staph)
...
# Compute class weights with a stronger focus on Bor, E, and Bacillus
class_weights = compute_class_weight(class_weight='balanced', classes=np.unique(y_train_encoded), y=y_train_encoded)
class_weights_dict = {i: class_weights[i] for i in range(len(class_weights))}
#class_weights_dict[expected_classes.index('Bor')] = 3.0  # Boosting weights for underperforming classes
class_weights_dict[expected_classes.index('Bor')] = 4.5  # Boosting weights for underperforming classes
class_weights_dict[expected_classes.index('E')] = 5.0
class_weights_dict[expected_classes.index('Bacillus')] = 2.5
...
param_grid = {
    ...
    'class_weight': [class_weights_dict]  # Use custom class weights
}
