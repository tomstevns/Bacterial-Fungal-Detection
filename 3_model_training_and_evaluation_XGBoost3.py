...
# Augmentation function with more techniques
def augment_image(img):
    # Apply random augmentations
    img_rotated = rotate(img, angle=random.uniform(-25, 25), mode='wrap')  # Rotate image
    img_brightened = exposure.adjust_gamma(img_rotated, gamma=random.uniform(0.5, 1.5))  # Adjust brightness
    img_rescaled = exposure.rescale_intensity(img_brightened, in_range='image', out_range=(0, 255))  # Rescale intensity
    return img_rescaled

# Function to load images and labels with augmentation
    ...
    for cls in expected_classes:
        ...
        for img_name in os.listdir(class_dir):
            ...
            try:
                ...
                if augment:
                    img_resized = augment_image(img_resized)  # Apply augmentations if enabled
...
# Define the parameter grid for hyperparameter tuning with smaller learning rate and more trees
param_grid = {
    'n_estimators': [100, 300, 500],  # More trees for better accuracy
    'max_depth': [6, 10, 20],  # Depth of each tree
    'learning_rate': [0.001, 0.01, 0.1],  # Smaller learning rates
    'colsample_bytree': [0.7, 1],  # Subsample ratio of columns
}
...
print("All results saved to 'xgboost_results.csv'")
