...
def summarize_epochs(df):
    ...
    summary = {
        ...
        "Epochs Overfitting Detected": detect_overfitting_epochs(df)  # Updated to capture all relevant epochs
    }

    return summary


# Function to detect all overfitting epochs
def detect_overfitting_epochs(df):
    ...
    for i in range(1, len(df)):
        if df['Val_Loss'].iloc[i] > df['Val_Loss'].iloc[i - 1] and df['Loss'].iloc[i] < df['Loss'].iloc[i - 1]:
            overfitting_epochs.append(df['Epoch'].iloc[i])
    if overfitting_epochs:
        return overfitting_epochs
    return None
...
def plot_epochs(df):
    ...
    # Highlight overfitting epochs if detected
    overfitting_epochs = detect_overfitting_epochs(df)
    if overfitting_epochs:
        for epoch in overfitting_epochs:
            plt.suptitle(f'Overfitting detected at Epochs {overfitting_epochs}', fontsize=16, color='red')
...
log_file_path = 'epoch56.log'  # Change to your actual log file path
